...
class RobocasaInputs(transforms.DataTransformFn):
    """Inputs for the Robocasa policy.
    ...
    def __call__(self, data: dict) -> dict:
        data = _decode_Robocasa(data, adapt_to_pi=self.adapt_to_pi)
...
class RobocasaOutputs(transforms.DataTransformFn):
    """Outputs for the Robocasa policy."""
...
def _joint_flip_mask() -> np.ndarray:
    """Used to convert between Robocasa and pi joint angles."""
...
def _gripper_to_angular(value):
    # Robocasa transforms the gripper positions into a linear space. The following code
    # reverses this transformation to be consistent with pi0 which is pretrained in
    # angular space.
    #
    # These values are coming from the Robocasa code:
...
def _gripper_from_angular(value):
    # Convert from the gripper position used by pi0 to the gripper position that is used by Robocasa.
...
def _decode_Robocasa(data: dict, *, adapt_to_pi: bool = False) -> dict:
...
def _decode_state(state: np.ndarray, *, adapt_to_pi: bool = False) -> np.ndarray:
    ...
    #     # Reverse the gripper transformation that is being applied by the Robocasa runtime.
